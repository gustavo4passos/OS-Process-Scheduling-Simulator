	...
	const std::vector<ProccessTemplate>& proccessList)
{
	...
	for(auto& proccess : proccessList)
	{
		...
		file.write((const char*)(&(proccess).numberOfPages), sizeof(unsigned));
	}
...
std::vector<ProccessTemplate> LoadProccessList(const std::string& filename)
{
	...
	while(file.peek() != EOF)
	{
		unsigned ID, duration, arrivalTime, deadline, numberOfPages;
		...
		file.read((char*)&numberOfPages, sizeof(unsigned));
		if(!file.good()) return std::vector<ProccessTemplate>();

		proccessList.push_back({ ID, arrivalTime, duration, deadline, numberOfPages });
	}
